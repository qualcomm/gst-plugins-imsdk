...
{
  gchar *title = (opt == REWIND_OPTION ? "Rewind" : "Fast Forward");
  gint spaces = (opt == REWIND_OPTION ? 24 : 21);
...
handle_ffr_menu (GstAppContext * appctx, gint * opt)
{
  ...
  gint input = 0;
  guint bitrate = 0;
  gint mul = (*opt == REWIND_OPTION ? -1 : 1);
...
main_menu (gpointer userdata)
{
  ...
  while (active) {
    if (opt == FAST_FORWARD_OPTION || opt == REWIND_OPTION)
