...
#define STANDBY_OPTION                 "s"
#define PLAYING_OPTION                 "p"
...
static void
send_camera_standby (GstElement * pipeline)
{
  // Create standby event
  GstEvent *cam_plug_event = gst_event_new_custom (GST_EVENT_CUSTOM_DOWNSTREAM,
      gst_structure_new_empty ("camera-standby"));

  // Send 'camera-standby' event to pipeline
  gst_element_send_event (pipeline, cam_plug_event);
...
{
  g_print ("\n%.25s MENU %.25s\n", DASH_LINE, DASH_LINE);
  g_print ("   (%s) %-25s\n", STANDBY_OPTION, "Switch to standby mode");
  g_print ("   (%s) %-25s\n", PLAYING_OPTION, "Switch to playing mode");
...
main_menu (gpointer userdata)
{
  ...
  if (!update_pipeline_state (appctx->pipeline,
      appctx->messages, GST_STATE_NULL)) {
  ...
  while (active) {
    ...
    } else if (g_str_equal (str, STANDBY_OPTION)) {
      send_camera_standby (appctx->pipeline);
      active = update_pipeline_state (appctx->pipeline,
          appctx->messages, GST_STATE_PAUSED);
    } else if (g_str_equal (str, PLAYING_OPTION)) {
      active = update_pipeline_state (appctx->pipeline,
          appctx->messages, GST_STATE_PLAYING);
    }
  }
...
main (gint argc, gchar *argv[])
{
  ...
  g_set_prgname ("gst-camera-standby-example");
