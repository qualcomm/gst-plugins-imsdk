 ...
 *                 |-> tee -> qtimetamux
 *                 |-> tee -> Pre process->
 *                 |-> ML Framework -> Post process -> qtimetamux
 *                 -> qtimetamux -> waylandsink (Display)
 *
...
create_pipe (GstAppContext * appctx, GstAppOptions * options)
{
  ...
  GstElement *qtimlqnn = NULL, *qtimetamux = NULL;
  ...
  qtimlqnn = gst_element_factory_make ("qtimlqnn","qtimlqnn");
  if (!qtimlqnn) {
  ...
  // 2.7 set the property of qtimlqnn
  g_object_set (G_OBJECT (qtimlqnn), "model", options->model_path, NULL);
  g_object_set (G_OBJECT (qtimlqnn), "backend", "/usr/lib/libQnnHtp.so", NULL);

  // 2.8 set the property of qtivoverlay
  g_value_init (&value, G_TYPE_INT);
  ...
  gst_bin_add_many (GST_BIN (appctx->pipeline), tee, qtimlvconverter, qtimetamux,
      qtimlqnn, qtimlvdetection, detection_filter, qtivoverlay, waylandsink,
      NULL);
  ...
  if (options->use_file) {
    ...
    ret = gst_element_link_many (tee, queue[2], qtimetamux, NULL);
    if (!ret) {
      g_printerr ("Pipeline elements cannot be linked for tee -> qtimetamux");
      goto error_clean_pipeline;
    }

    ret = gst_element_link_many (qtimetamux, queue[3], qtivoverlay, queue[4],
        waylandsink, NULL);
    if (!ret) {
      g_printerr ("Pipeline elements cannot be linked for"
          "qtimetamux->waylandsink\n");
      goto error_clean_pipeline;
    }

    ret = gst_element_link_many (
        tee, queue[5], qtimlvconverter, queue[6], qtimlqnn, queue[7],
        qtimlvdetection, detection_filter, queue[8], qtimetamux, NULL);
    if (!ret) {
      g_printerr ("Pipeline elements cannot be linked for"
    ...
    ret = gst_element_link_many (tee, queue[1], qtimetamux, NULL);
    if (!ret) {
      g_printerr ("Pipeline elements cannot be linked for tee -> qtimetamux");
      goto error_clean_pipeline;
    }

    ret = gst_element_link_many (qtimetamux, queue[2], qtivoverlay, queue[3],
        waylandsink, NULL);
    if (!ret) {
      g_printerr ("Pipeline elements cannot be linked for"
          "qtimetamux->waylandsink\n");
    ...
    ret = gst_element_link_many (
        tee, queue[4], qtimlvconverter, queue[5], qtimlqnn, queue[6],
        qtimlvdetection, detection_filter, queue[7], qtimetamux, NULL);
    if (!ret) {
      g_printerr ("Pipeline elements cannot be linked for"
          "pre proc -> ml framework -> post proc.\n");
      goto error_clean_pipeline;
...
error_clean_elements:
  cleanup_gst (&qtiqmmfsrc, &filesrc, &rtspsrc, &tee, &qmmfsrc_caps_preview,
      ...
      &qtimlqnn, &qtimetamux, &qtimlvdetection, &qtivoverlay, &detection_filter,
      &v4l2h264dec_caps, &waylandsink, NULL);
  for (gint i = 0; i < QUEUE_COUNT; i++) {
