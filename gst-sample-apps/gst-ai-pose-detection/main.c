...
create_pipe (GstAppContext * appctx, GstAppOptions * options)
{
  ...
  GstElement *qtimlvconverter = NULL, *qtimltflite = NULL;
  ...
  qtimltflite = gst_element_factory_make ("qtimltflite",
      "qtimltflite");
  if (!qtimltflite) {
  ...
  if (options->use_file) {
    ...
    if (options->camera_type == GST_CAMERA_TYPE_PRIMARY) {
      filtercaps = gst_caps_new_simple ("video/x-raw",
          ...
          "framerate", GST_TYPE_FRACTION, DEFAULT_CAMERA_FRAME_RATE, 1,
  ...
  gst_bin_add_many (GST_BIN (appctx->pipeline), tee, qtimlvconverter,
      qtimltflite, qtimlvpose, pose_filter,
  ...
  ret = gst_element_link_many (
      ...
      qtimltflite, queue[6], qtimlvpose,
...
error_clean_elements:
  cleanup_gst (&qtiqmmfsrc, &qmmfsrc_caps, &filesrc, &qtdemux,
      ...
      &qtimltflite, &qtimlvpose, &qtivcomposer, &pose_filter,
