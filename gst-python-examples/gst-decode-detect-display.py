...
Initializes and links elements for decoding from offline source INPUT,
performs inference (object detection) using MODEL and LABELS files,
and renders video on display.
"""
DEFAULT_INPUT_FILE = "/opt/data/1280_720_h264_30fps_1min.mp4"
...
def create_pipeline(pipeline):
    ...
    elements = {
        ...
        "tensor_caps" : create_element("capsfilter", "tensor_caps")
    }
    ...
    # Set properties
    elements["filesrc"].set_property("location", args.input)

    elements["parse"].set_property("config-interval", 1)
    ...
    elements["tensor_caps"].set_property("caps",
        Gst.Caps.from_string("text/x-raw")
    )

    elements["display"].set_property("fullscreen", True)
...
def main():
    ...
    # Create the pipeline
    try:
        pipeline = Gst.Pipeline.new("decode-detect-display-pipeline")
        if not pipeline:
